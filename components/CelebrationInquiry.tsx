...
export default function CelebrationInquiry() {  
  ...
  const selectedJourney = searchParams.get('journey') || '';
    ...
    try {  
      const response = await fetch('/api/lead', {  
        ...
        body: JSON.stringify({  
          ...
          notes: form.notes,  
          journeyConcept: selectedJourney,  
          source: selectedJourney  
            ? `Celebrating Her ,  ${selectedJourney}`  
            : 'Celebrating Her ,  The Celebration Collection',  
        }),  
      });
  ...
  return (  
    ...
    >  
      {selectedJourney && (  
        <p className="border-l border-[#8A6A2D] pl-4 font-sans text-[11px] uppercase tracking-[0.18em] text-[#071421]/55">  
          Inquiry regarding {selectedJourney}  
        </p>  
      )}
      ...
      <div className="grid gap-8 md:grid-cols-2">  
        ...
        <label className="block">  
          ...
          <input  
            ...
            placeholder="Best number for a planning conversation"  
            className={inputClassName}  
          />  
        </label>
      ...
      <input type="hidden" name="journeyConcept" value={selectedJourney} />
      ...
      >  
        {status === 'success' && (  
          <>  
            Thank you. We received your celebration inquiry and will be in  
            touch within one business day.{' '}  
            <a  
              href="https://cal.com/nexvoyagecollective/discovery-call"  
              target="_blank"  
              rel="noopener noreferrer"  
              className="underline decoration-[#8A6A2D]/40 underline-offset-4 hover:text-[#8A6A2D]"  
            >  
              Prefer to speak sooner? Schedule a call.  
            </a>  
          </>  
        )}
