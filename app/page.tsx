  ...
  <main className="bg-black text-white min-h-screen">  
    ...
    <style dangerouslySetInnerHTML={{ __html: `  
      * { background-color: transparent !important; box-shadow: none !important; }  
      ...
      .border-y { border-top: 1px solid rgba(255,255,255,0.1) !important; border-bottom: 1px solid rgba(255,255,255,0.1) !important; }  
    `}} />

    {/* Hero Section - Now in Full Color */}  
    ...
    {/* Partner Alliance - 15 Pillars Tightened */}  
    <section className="py-8 border-y border-white/10">  
      <div className="max-w-5xl mx-auto px-6">  
        <div className="grid grid-cols-3 md:grid-cols-5 gap-y-6 gap-x-4 opacity-70 items-center text-center">  
          {partners.map((partner) => (  
            <span key={partner.name} className={`text-white text-[10px] md:text-[12px] ${partner.style}`}>  
              {partner.name}  
            </span>  
          ))}  
        </div>  
    ...
    {/* Property Showcase - Gray updated to Brass */}  
