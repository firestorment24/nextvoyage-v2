...
excerpt: "A curated winter pilgrimage through the historic alleyways of Central Europe, featuring private after-hours artisan access and fireside heritage estates.",       
...
excerpt: "Navigating the historic waters of Southeast Asia aboard Heritage Line boutique river vessels with complete privacy and bespoke land choreography.",       
...
excerpt: "An unhurried exploration of Venetian heritage uniting Villa Barbarich with private, behind-the-scenes access to the masters of Homo Faber.",       
...
<div className="flex justify-between items-end mb-16">          
<div>          
<h2 className="text-[#D4AF37] text-4xl font-serif font-light tracking-tight">From the Perspective</h2>          
<p className="text-white/40 text-xs uppercase tracking-[0.3em] mt-4 font-sans">          
Reflections from the editorial desk — essays, intelligence &amp; itineraries          
</p>          
</div>          
<Link href="/perspective" className="text-white/60 text-xs uppercase tracking-widest border-b border-[#D4AF37] pb-1 hover:text-white transition-colors font-sans">          
View All Editorial          
</Link>          
</div>          
<div className="grid md:grid-cols-3 gap-12">          
{editorial.map((item, i) => (          
<Link key={i} href={`/perspective/${item.slug}`} className="space-y-4 group cursor-pointer">          
  <div className="relative aspect-[4/5] overflow-hidden bg-white/5">          
    <img src={item.image} alt={item.title} className="absolute inset-0 w-full h-full object-cover transition-all duration-700 group-hover:scale-105" />          
    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors duration-500" />          
  </div>          
  <div className="space-y-3">          
    <span className="text-[#D4AF37] text-[9px] uppercase tracking-widest font-sans">{item.category}</span>          
    <h3 className="text-white text-xl font-light leading-snug group-hover:text-[#D4AF37] transition-colors duration-300">{item.title}</h3>          
    <p className="text-white/50 text-sm leading-relaxed line-clamp-3 font-sans font-light">{item.excerpt}</p>          
    <p className="text-white/30 text-[8px] uppercase tracking-[0.2em] font-sans">{item.meta}</p>          
  </div>          
</Link>          
))}          
</div>          
