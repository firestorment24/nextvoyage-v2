...
export default function AdminPage() {  
  ...
  const stats = {  
    ...
    pending: dossiers.filter((d) => d.status === 'Pending' || d.status === 'New').length,  
    contacted: dossiers.filter((d) => d.status === 'Contacted').length,  
    ...
    return (  
      <div style={{  
        ...
        <form onSubmit={handleLogin} style={{  
          ...
          />  
          {error && (  
            <p style={{ color: '#FF4A4A', fontSize: '0.8rem', marginTop: '0.5rem' }}>{error}</p>  
          )}  
  ...
  if (selectedDossier) {  
    return (  
      <div style={{  
        ...
        <button onClick={() => setSelectedDossier(null)} style={{  
          background: 'none',  
          border: '1px solid #2A2A2A',  
          borderRadius: '4px',  
          color: '#D4AF37',  
        ...
        <div style={{  
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>  
            {[  
              ...
              { label: 'Submitted', value: new Date(selectedDossier.created_at).toLocaleDateString('en-US', {  
          ...
          {selectedDossier.notes && (  
            <div style={{ marginTop: '1.5rem', borderTop: '1px solid #2A2A2A', paddingTop: '1rem' }}>  
              <p style={{ color: '#666', fontSize: '0.75rem', margin: '0 0 0.25rem', letterSpacing: '0.1em' }}>NOTES</p>  
              <p style={{ margin: 0, fontSize: '0.9rem', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{selectedDossier.notes}</p>  
            </div>  
          )}  
        </div>

        <button onClick={() => { setIsAuthenticated(false); setPassword(''); setSelectedDossier(null) }} style={{  
          marginTop: '2rem',  
          background: '#1A1A1A',  
          border: '1px solid #2A2A2A',  
          borderRadius: '4px',  
  ...
  return (  
    <div style={{  
      ...
      <div style={{  
        ...
        <div>  
          <h1 style={{  
            ...
            margin: 0,  
            letterSpacing: '0.1em',  
          }}>NEXVOYAGE COLLECTIVE · ADMIN</h1>  
          <p style={{ color: '#666', fontSize: '0.75rem', margin: '0.25rem 0 0', letterSpacing: '0.1em' }}>  
            {dossiers.length} DOSSIER{dossiers.length !== 1 ? 'S' : ''} ON FILE  
          </p>  
        </div>  
      </div>
      ...
      <div style={{  
        ...
        <input  
          type="text"  
          placeholder="Search by name, email, destination, or occasion..."  
          value={searchQuery}  
        ...
        >  
          {['All', 'New', 'Pending', 'Contacted', 'Accepted', 'Declined'].map((s) => (  
      ...
      <div style={{  
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>  
          ...
          <tbody>  
            {fetching ? (  
              <tr>  
                <td colSpan={8} style={{ padding: '3rem', textAlign: 'center', color: '#666' }}>Loading dossiers...</td>  
              </tr>  
              ...
              filteredDossiers.map((dossier) => (  
                <tr key={dossier.id} style={{ borderBottom: '1px solid #2A2A2A' }}>  
                  ...
                  <td style={{ padding: '0.75rem 1rem' }}>  
                    <span style={{  
                      background: `${statusColors[dossier.status] || '#666'}20`,  
                      color: statusColors[dossier.status] || '#666',  
                  ...
                  <td style={{ padding: '0.75rem 1rem', fontSize: '0.75rem', color: '#666' }}>  
                    {new Date(dossier.created_at).toLocaleDateString('en-US', {  
                      month: 'short', day: 'numeric', year: 'numeric',  
                    })}  
                  </td>  
                  <td style={{ padding: '0.75rem 1rem' }}>  
                    <button onClick={() => setSelectedDossier(dossier)} style={{  
                      background: 'none',  
