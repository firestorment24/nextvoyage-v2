...
// Filter  
const [activeFilters, setActiveFilters] = useState<string[]>(['New', 'Contacted', 'In Discussion', 'Accepted', 'Declined']);
...
const saveNotes = async () => {  
  ...
  setSavingNotes(true);  
  try {  
    const res = await fetch(`/api/lead/${selected.id}`, {  
      method: 'PATCH',  
      headers: { 'Content-Type': 'application/json' },  
    ...
    if (res.ok) {  
      setDossiers(prev =>  
        prev.map(d => (d.id === selected.id ? { ...d, notes: editNotes } : d))  
      );  
...
const filteredDossiers = dossiers.filter(d => activeFilters.includes(d.status));
  ...
  <div style={{  
    ...
    <main style={{ padding: '32px', maxWidth: '1200px', margin: '0 auto' }}>  
      ...
      <div style={{  
        ...
        <span style={{ color: '#555', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px', marginRight: '8px' }}>  
          Filter:  
        </span>  
        {statusOptions.map(s => {  
          ...
          return (  
            <button  
              key={s}  
              onClick={() => {  
                if (activeFilters.length === 1 && activeFilters[0] === s) return;  
              ...
              style={{  
                ...
                ...(isActive  
                  ? {  
                      background:  
                        s === 'New' ? 'rgba(212, 175, 55, 0.15)' :  
                        s === 'Contacted' ? 'rgba(96, 165, 250, 0.15)' :  
                        s === 'In Discussion' ? 'rgba(167, 139, 250, 0.15)' :  
                        s === 'Accepted' ? 'rgba(74, 222, 128, 0.15)' :  
        ...
        {activeFilters.length < 5 && (  
          <button  
            onClick={() => setActiveFilters(['New', 'Contacted', 'In Discussion', 'Accepted', 'Declined'])}  
            style={{  
              background: 'none',  
              border: 'none',  
              color: '#555',  
      ...
      <div style={{  
        ...
        <div style={{  
          ...
          <span style={{ color: '#555', fontSize: '13px' }}>  
            {filteredDossiers.length} of {dossiers.length} shown  
          </span>  
        </div>
            ...
            <tbody>  
              {filteredDossiers.length === 0 ? (  
                ...
                filteredDossiers.map(d => (  
                  <tr  
                    key={d.id}  
                    onClick={() => { setSelected(d); setEditNotes(d.notes || ''); }}  
                    style={{  
                  ...
                  >  
                    ...
                    <td style={{ padding: '14px 20px' }}>  
                      <button  
                        onClick={e => { e.stopPropagation(); setSelected(d); setEditNotes(d.notes || ''); }}  
                        style={{  
                          background: 'none',  
                          border: '1px solid #333',  
                          color: '#aaa',  
          ...
          <div style={{ padding: '24px' }}>  
            ...
            <div style={{ marginBottom: '24px' }}>  
              ...
              <textarea  
                ...
                placeholder="Add private notes about this dossier..."  
                style={{  
                  width: '100%',  
                  minHeight: '80px',  
                  padding: '12px',  
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '8px' }}>  
                <button  
                  disabled={savingNotes || editNotes === (selected.notes || '')}  
                  onClick={saveNotes}  
                  style={{  
                    padding: '8px 18px',  
                    borderRadius: '6px',  
                    border: '1px solid #D4AF37',  
                    background: savingNotes ? 'rgba(212, 175, 55, 0.1)' : 'transparent',  
                    color: '#D4AF37',  
                    fontSize: '12px',  
                    cursor: savingNotes ? 'default' : 'pointer',  
                    opacity: savingNotes || editNotes === (selected.notes || '') ? 0.5 : 1,  
                  }}  
                >  
