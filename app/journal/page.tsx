...
export default function JournalPage() {  
  return (  
    <main className="min-h-screen bg-[#050505] text-[#E5E5E5] selection:bg-zinc-800 selection:text-white relative">  
      ...
      <header className="relative pt-24 md:pt-32 pb-16 md:pb-20 px-6 md:px-12 max-w-7xl mx-auto border-b border-zinc-900/50 z-10">  
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">  
          <div>  
            <p className="font-mono text-[9px] md:text-[10px] tracking-[0.3em] text-zinc-500 uppercase mb-4">  
              Volume 01 // The Master Ledger  
            </p>

            <h1 className="text-5xl sm:text-6xl md:text-8xl font-serif tracking-tighter leading-none italic m-0">  
              The Journal  
            </h1>  
          </div>
      ...
      <section className="relative px-6 md:px-12 max-w-7xl mx-auto py-16 md:py-24 z-10">  
            ...
            >  
              <div className="relative aspect-[4/5] overflow-hidden bg-zinc-900 grayscale-0 group-hover:grayscale transition-all duration-700 ease-in-out">  
                <img  
                  src={item.heroImage}  
                  alt={item.title}  
                  className="object-cover w-full h-full scale-100 group-hover:scale-105 transition-transform duration-1000"  
                />
              ...
              <div className="space-y-4">  
                <div className="space-y-1">  
                  <h2 className="text-xl md:text-2xl font-serif group-hover:italic transition-all duration-300 m-0">  
                    {item.title}  
                  </h2>

                  <p className="font-mono text-[9px] tracking-[0.2em] text-zinc-500 uppercase">  
                    Field Report No. {item.slug}  
                  </p>  
                </div>
                ...
                <div className="pt-4 border-t border-zinc-900 flex justify-between items-center text-[9px] font-mono tracking-widest uppercase text-zinc-600">  
                  ...
                  <span className="hidden md:block opacity-0 group-hover:opacity-100 transition-opacity duration-500">  
                    View Archive —&gt;  
                  </span>  
                </div>  
              </div>  
      ...
      <footer className="relative py-16 md:py-20 px-6 md:px-12 max-w-7xl mx-auto border-t border-zinc-900/50 flex flex-col md:flex-row justify-between items-center gap-6 text-[9px] md:text-[10px] font-mono tracking-[0.3em] text-zinc-500 uppercase z-10">  
        <div>End of Ledger // Page 01 of {DESTINATIONS_DATA.length}</div>

        <Link  
          href="/inquiry"  
          className="hover:text-white transition-colors underline decoration-zinc-800 underline-offset-8"  
        >  
          Begin Consultation  
        </Link>  
      </footer>  
